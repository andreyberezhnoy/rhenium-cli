...
const run = async () => {
  ...
  const userDeps = ['gulp-postcss', 'autoprefixer', 'gulp-sourcemaps'];
  ...
  try {
    ...
    await fs.copySync(
      ...
      path.resolve(process.cwd(), projectName, 'config/')
    );
    await fs.copySync(
  ...
  console.log(
    `${chalk.yellow('ℹ')} Installing Gulp, necessary plugins ${chalk.hex(
      '#FE9920'
    )(depsToInstall)}...`
  );
  console.log('');
